...
class KongOAuthPermissionTestCase(TestCase):
    ...
    def test_get_request_has_permissions(self):
        ...
        self.assertTrue(response.status_code == 200, msg="Valid Permissions Fail")

    def test_patch_request_has_permissions(self):
        response = self.client.patch(
            "/example/{}/".format(self.example.id), **self.headers
        )
        self.assertTrue(response.status_code == 403, msg="Patch has no permissions")
    ...
    def test_post_has_invalid_permissions(self):
        response = self.client.post(
            "/example/", data={"name": "example"}, **self.headers
        )
        self.assertTrue(response.status_code == 403, msg="Invalid Permissions Passes")
